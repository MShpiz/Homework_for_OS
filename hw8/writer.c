...
int main() {
  ...
  // Формирование объекта разделяемой памяти для общего доступа к кольцевому
  // буферу
  ...
  buffer = mmap(0, sizeof(shared_memory), PROT_WRITE | PROT_READ, MAP_SHARED,
                buf_id, 0);
  ...
  // Перед доступом читателя, который обрабатывает натуральные числа в
  // ограниченном диапазоне
  for (int i = 0; i < BUF_SIZE; ++i) {
    buffer->store[(i + lastIdx + 1) % BUF_SIZE] = -1;
  }
  ...
  while (1) {
    ...
    // Поиск первой свободной ячейки для записи. Она должна быть. Иначе сюда не
    // попасть
    int i = 0;
    while (buffer->store[(i + lastIdx + 1) % BUF_SIZE] != -1) {
      printf("    --> checking store: store[%d] = %d\n",
             (i + lastIdx + 1) % BUF_SIZE,
             buffer->store[(i + lastIdx + 1) % BUF_SIZE]);
      ++i;
    }
    // Запись в первую свободную ячейку
    buffer->store[(i + lastIdx + 1) % BUF_SIZE] =
        rand() % 11; // число от 0 до 10
