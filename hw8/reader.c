...
int main() {
  ...
  while (1) {
    ...
    while(buffer->store[(i+lastIdx+1)%BUF_SIZE] == -1) {
      printf("    --> checking store: store[%d] = %d\n", (i+lastIdx+1)%BUF_SIZE, buffer->store[(i+lastIdx+1)%BUF_SIZE]);
      ++i;
    }
    // Получение значения из читаемой ячейки
    int result = buffer->store[(i+lastIdx+1)%BUF_SIZE];
    buffer->store[(i+lastIdx+1)%BUF_SIZE] = -1;  // данные прочитаны
    ...
    printf("Consumer %d: Reads value = %d from cell [%d], factorial = %d\n",
           pid, result, (i+lastIdx+1)%BUF_SIZE, f) ;
